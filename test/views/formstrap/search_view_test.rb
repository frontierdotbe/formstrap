    ...
    describe "with prepend" do
      before do
        visit_inline locals: {post: @post} do
          <<~ERB
            <%= formstrap_form_with model: post do |form| %>
              <%= form.search :title, prepend: "lorem" %>
            <% end %>
          ERB
        end
      end
    ...
    describe "with append" do
      before do
        visit_inline locals: {post: @post} do
          <<~ERB
            <%= formstrap_form_with model: post do |form| %>
              <%= form.search :title, append: "lorem" %>
            <% end %>
          ERB
        end
      end
    ...
    describe "with wrapper" do
      before do
        visit_inline locals: {post: @post} do
          <<~ERB
            <%= formstrap_form_with model: post do |form| %>
              <%= form.search :title, wrapper: {class: "lorem", data: {test: 1}} %>
            <% end %>
          ERB
        end
      end
