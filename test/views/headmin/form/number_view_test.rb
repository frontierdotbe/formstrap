...
module Headmin
  module Form
    class NumberViewTest < ViewTestCase
      before do
        @post = Post.new
      end
          ...
          visit_inline locals: {post: @post} do
            <<~ERB
              <%= form_with model: post do |form| %>
                <%= render "headmin/forms/number", form: form, attribute: :title %>
              <% end %>
            ERB
          end
        end
      ...
      describe "with float" do
        before do
          visit_inline locals: {post: @post} do
            <<~ERB
              <%= form_with model: post do |form| %>
                <%= render "headmin/forms/number", form: form, attribute: :title, float: true %>
              <% end %>
            ERB
          end
        end
      ...
      describe "with prepend" do
        before do
          visit_inline locals: {post: @post} do
            <<~ERB
              <%= form_with model: post do |form| %>
                <%= render "headmin/forms/number", form: form, attribute: :title, prepend: "lorem" %>
              <% end %>
            ERB
          end
        end
      ...
      describe "with append" do
        before do
          visit_inline locals: {post: @post} do
            <<~ERB
              <%= form_with model: post do |form| %>
                <%= render "headmin/forms/number", form: form, attribute: :title, append: "lorem" %>
              <% end %>
            ERB
          end
        end
      ...
      describe "with label" do
        before do
          visit_inline locals: {post: @post} do
            <<~ERB
              <%= form_with model: post do |form| %>
                <%= render "headmin/forms/number", form: form, attribute: :title, label: "lorem" %>
              <% end %>
            ERB
          end
        end
      ...
      describe "with list" do
        before do
          visit_inline locals: {post: @post} do
            <<~ERB
              <%= form_with model: post do |form| %>
                <%= render "headmin/forms/number", form: form, attribute: :title, list: [1,2,3] %>
              <% end %>
            ERB
          end
        end
      ...
      describe "with wrapper" do
        before do
          visit_inline locals: {post: @post} do
            <<~ERB
              <%= form_with model: post do |form| %>
                <%= render "headmin/forms/number", form: form, attribute: :title, wrapper: {class: "lorem", data: {test: 1}} %>
              <% end %>
            ERB
          end
        end
